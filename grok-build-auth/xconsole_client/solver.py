    ...
    def solve_turnstile(
        ...
        *,
        ...
        Args:
            ...
            proxy: Optional proxy URL for local Camoufox (``captcha_use_proxy``).
                When set, createTask uses type ``TurnstileTask`` with proxy so
                the browser egress matches registration protocol. Remote
                YesCaptcha Proxyless workers ignore this (their own IP).
        ...
        # Inline local Camoufox only implements one Turnstile path. Skip M1 there so we
        ...
        # Local + proxy: use TurnstileTask (with proxy field). Local without proxy:
        # Proxyless (server egress). Remote YesCaptcha stays Proxyless.
        use_local_proxy = bool(local and proxy_url)

        task_types: list[str] = []
        if use_local_proxy:
            task_types.append("TurnstileTask")
        elif use_premium:
        ...
        for idx, task_type in enumerate(task_types):
            ...
            if use_local_proxy and proxy_url:
                task["proxy"] = proxy_url
            try:
                proxy_tag = (
                    "proxy=job"
                    if use_local_proxy
                    else ("proxy=no" if local else "proxy=remote-worker")
                )
                self._progress(
