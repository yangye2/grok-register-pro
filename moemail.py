...
from register_pro_config import (
...
def normalize_mail_provider(provider: str | None, *, base_url: str | None = None) -> str:
    ...
    """Return ``moemail`` | ``yyds`` | ``gptmail`` | ``cfmail`` | ``duckmail`` | ``outmail``.
    ...
    if p in {"outmail", "outlook", "outlookemail", "outlook-mail", "outlook_mail"}:
    ...
    if any(x in base for x in ("outmail", "/api/accounts", "temp-emails")):

        # weak heuristic; prefer explicit provider=outmail
...
def random_mailbox_local(min_len: int = 6, max_len: int = 14) -> str:
    """Generate a human-ish mailbox local-part (not fixed-width hex digits).

    Rules:
    - variable length within [min_len, max_len]
    - starts with a letter
    - mixed letters/digits (and occasional dot/underscore)
    - avoids pure numeric / pure hex fingerprints
    """
    lo = max(3, int(min_len or 6))
    hi = max(lo, min(32, int(max_len or 14)))
    n = secrets.randbelow(hi - lo + 1) + lo
    letters = string.ascii_lowercase
    digits = string.digits
    alnum = letters + digits
    words = (
        "neo", "sam", "kai", "lee", "rio", "sky", "nova", "luna", "orion", "atlas",
        "mira", "axel", "jade", "iris", "cole", "reed", "zane", "vera", "owen", "elise",
        "hugo", "nina", "cleo", "aria", "milo", "theo", "lark", "wren", "sage", "finn",
    )
    vowels = "aeiou"
    consonants = "bcdfghjklmnpqrstvwxyz"

    style = secrets.randbelow(6)
    if style == 0:
        # word + random tail
        base = secrets.choice(words)
        tail_n = max(2, n - len(base))
        tail = "".join(secrets.choice(alnum) for _ in range(tail_n))
        local = (base + tail)[:n]
    elif style == 1:
        # syllable-ish name + digits
        parts = []
        while len("".join(parts)) < max(3, n - 3):
            parts.append(secrets.choice(consonants) + secrets.choice(vowels))
            if secrets.randbelow(3) == 0:
                parts.append(secrets.choice(consonants))
        core = "".join(parts)[: max(3, n - 2)]
        local = core + "".join(secrets.choice(digits) for _ in range(n - len(core)))
    elif style == 2:
        # letter run with embedded digits
        chars = [secrets.choice(letters)]
        for _ in range(n - 1):
        ...
        # dotted local: a.bcd12
        left_n = max(2, min(n - 3, secrets.randbelow(4) + 2))
        right_n = max(2, n - left_n - 1)
        left = "".join(secrets.choice(letters) for _ in range(left_n))
        right = "".join(secrets.choice(alnum) for _ in range(right_n))
        local = f"{left}.{right}"[:n]
        if local.startswith("."):
            local = secrets.choice(letters) + local[1:]
        if local.endswith("."):
            local = local[:-1] + secrets.choice(alnum)
    elif style == 4:
        # underscore style: name_xx12
        base = secrets.choice(words)
        rest_n = max(2, n - len(base) - 1)
        rest = "".join(secrets.choice(alnum) for _ in range(rest_n))
        local = f"{base}_{rest}"[:n]
    else:
        # mixed alnum, letter start, avoid pure hex look by forcing a non-hex letter sometimes
        non_hex_letters = "ghijklmnopqrstuvwxyz"
        chars = [secrets.choice(letters)]
        for _ in range(n - 1):
            chars.append(secrets.choice(alnum))
        if all(c in "0123456789abcdef" for c in chars):
            chars[secrets.randbelow(len(chars))] = secrets.choice(non_hex_letters)
        local = "".join(chars)

    local = re.sub(r"[^a-z0-9._-]", "", local.lower())
    local = local.strip("._-")
    if not local or not local[0].isalpha():
        local = secrets.choice(letters) + local
    if len(local) < lo:
        local = local + "".join(secrets.choice(alnum) for _ in range(lo - len(local)))
    if len(local) > hi:
        local = local[:hi]
    # final guard: never end with separator
    local = local.rstrip("._-")
    if len(local) < 3:
        local = (local + "".join(secrets.choice(alnum) for _ in range(3)))[: max(3, lo)]
    return local


def secrets_token_hex_local() -> str:
    """Backward-compatible alias for random mailbox local-part generator."""
    return random_mailbox_local()
...
def duckmail_create_mailbox(
    ...
    *,
    ...
    local = (name or "").strip().lower() or random_mailbox_local()
    ...
    *,
    ...
    if prov == "outmail":
        ...
        if base_url:
            ...
            om_cfg["outmail_api_base"] = base_url

        if api_key:

            om_cfg["outmail_api_key"] = api_key

        try:

            import register_pro_store as _store

            reg = _store.get_registration_config(include_secrets=True)
        ...
        email_addr, token = om.outmail_get_email_and_token()
...
def fetch_messages(
    ...
    *,
    ...
    if prov == "outmail":
        ...
        # Outmail polling is handled by outmail_client via wait_for_code.

        # Returning empty list keeps generic pollers from erroring.
